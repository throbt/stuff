    ...
    $(['#',id].join('')).html([
      '<div id="datepicker_wrapper">',
        '<div id="calendar_header">',
          '<div id="left_control_month_date" rel="month-" class="picker_control_date">',
          '</div>',
          '<div id="left_control_year_date" rel="year-" class="picker_control_date">',
          '</div>',
          '<div id="display" class="picker_display">',
            '<p id="thisMonth_booking">',
              this.getMonth()[this.date.month],
            '</p>',
            '<p id="thisYear_booking">',
              this.date.year,
            '</p>',
          '</div>',
          '<div id="right_control_year_date" rel="year+" class="picker_control_date">',
          '</div>',
          '<div id="right_control_month_date" rel="month+" class="picker_control_date">',
          '</div>',
        '</div>',
        '<table>',
          '<thead>',
      ...
      if(i != 0) {
        if(dayCounter == 1) {
          ...
          if(i == this.date.day && this.dateCurrent.month == this.date.month && this.dateCurrent.year == this.date.year)
            cells.push(['<td class="currentDate"><span class="picker currentday">',currDay,'</span></td>'].join(''));
          else
            cells.push(['<td><span class="picker day">',currDay,'</span></td>'].join(''));
