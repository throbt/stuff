...
$(document).ready(function() {
  ...
  $('.saveGallery').live('click',function() {
    ...
    if(mainId == '') {
      ...
      $.get(
        '/admin_ajax/saveGalleryItem',
        {'id':mainId,'data':dataSave},
        function(resp) {
          window.location.href = '/node/nodetype/gallery';
        }
      );
    }
    return false;
  ...
  if($.inArray(id,dataSave) == -1) {
    $('#siderElements').append(
      ['<div class="span12" id="element'+id+'" style="margin-top:4px;">',
        ...
        '</div>',
        '<div class="span2">',
          '<a id="" class="btn btn-primary deleteEl" rel="'+id+'" href="" type="submit">Töröl</a>',
        '</div>',
      '</div>'].join('')
    );
    dataSave.push(id);
...
var addSelectContent = function() {
  ...
  $.each(data, function(i) {
    $('#selectContentShow').append(
      ['<option value="',data[i]['nid'],'">',data[i]['type'],(data[i]['galleryTitle'] ? [' | ',data[i]['galleryTitle']].join('') : ''),' | ',data[i]['title'],'</option>'].join('')  //' | ',data[i]['galleryTitle']
    );
    $('#addSelectContent').append(
      ['<option value="',data[i]['nid'],'">',data[i]['type'],(data[i]['galleryTitle'] ? [' | ',data[i]['galleryTitle']].join('') : ''),' | ',data[i]['title'],'</option>'].join('')
    );
  });
...
<ul class="breadcrumb">
  ...
  <li>
    <a href="/node/nodetype/gallery">Gallery</a>
    <span class="divider">/</span>
  </li>
  <li class="active">
...
<div class="row show-grid">
  <div class="span12">
    <div class="span6">
        <a id="" class="btn btn-primary saveGallery" rel="" href="" type="submit">Ment</a>
    </div>
  </div>
</div>  
